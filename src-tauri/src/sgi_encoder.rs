...
pub fn write_sgi(
    ...
    // get the file writer
    let result_file =
        File::create(path).map_err(|e| format!("Failed to open base image: {}", e))?;
    ...
    for z in 0..4 {
        for y in (0..height).rev() {
            // FLIP ORDER BECAUSE SGI starts counting at bottom left of image
            for x in 0..width {
...
fn get_header(width: u16, height: u16, rle_compression: bool) -> [u8; 512] {
    ...
    header[16..20].copy_from_slice(&255u32.to_be_bytes()); // pixelmax 255
                                                           // 4 bytes dummy
    let image_name = b"hello world...\0";
    header[24..24 + image_name.len()].copy_from_slice(image_name); // image name
