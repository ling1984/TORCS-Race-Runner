...
pub fn change_banners(banner_path: &str) {
    // Match Python's os.path.dirname(os.path.realpath(sys.argv[0]))
    let mut corkscrew_path = std::env::current_exe()
        .ok()
        .and_then(|p| p.parent().map(|p| p.to_path_buf()))
        .unwrap_or_else(|| PathBuf::from("."));
    
    corkscrew_path.push("torcs");
    corkscrew_path.push("tracks");
    corkscrew_path.push("road");
    corkscrew_path.push("corkscrew");
    ...
    if banner_path.is_empty() {
        ...
        for (i, coords_list) in truck_coords.iter().enumerate() {
            ...
            for &pos in coords_list {
                let banner_img = match image::open(banner_img_path) {
                    Ok(img) => img.to_rgba8(),
                    Err(_) => continue,
                };
                ...
                if loop_count == 0 {
                    ...
                    let flipped = image::imageops::flip_horizontal(&banner_img);
                    let resized = image::imageops::resize(&flipped, res.0, res.1, FilterType::Nearest);
        ...
        for i in 0..files.len() {
            ...
            if let Ok(banner) = image::open(banner_img_path) {
                let resized = image::imageops::resize(&banner.to_rgba8(), target_res[i].0, target_res[i].1, FilterType::Nearest);
